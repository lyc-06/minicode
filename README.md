...
这个项目源于一道笔试题：**从零实现一个最小可用 Agent**。

| 要求 | 实现 |
|------|------|
| 多轮对话 + session 维护 | ✅ 消息队列 + JSONL 持久化 |
| 不依赖现成 Agent 框架 | ✅ 核心 runtime 完全自实现 |
| Agent 循环（输入→判断→调工具→读结果→循环） | ✅ 75 行核心 loop |
| ≥3 个工具 | ✅ 14 个 |
| 最大步数限制 | ✅ 默认 25 步 |
| 异常处理 | ✅ HTTP 重试 + 空响应重试 + try/except |
| 工具调用 trace | ✅ TUI 实时显示 |
| 跨轮次继续执行 | ✅ 记忆系统 + Session 持久化 |
| 真实 LLM API | ✅ DeepSeek + Anthropic 双适配器 |
